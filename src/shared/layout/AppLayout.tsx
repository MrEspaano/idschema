    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      <header className="sticky top-0 z-50 border-b border-border/80 bg-card/95 backdrop-blur">
        <div className="mx-auto flex w-full max-w-3xl items-center justify-between px-4 py-3 sm:px-6">
          <Link to={isAdminRoute ? "/admin" : "/"} className="group flex items-center gap-2">
      ...
      <main className={cn("mx-auto w-full max-w-3xl flex-1 px-4 py-6 sm:px-6", isAdminRoute ? "pb-6" : "pb-24")}>
        {children}
      </main>
